...
4.  **Integración ENSO:** Clasifica el estado del fenómeno de El Niño/Niña según anomalías térmicas y genera variables categóricas con rezagos temporales. Estas 16 dummies (4 categorías × 4 lags) se comprimen posteriormente en 2 componentes PCA en la etapa S3.5.
...
6.  **Generación de Lags:** Crea variables de rezago temporales para todas las variables climáticas y ambientales, permitiendo capturar el efecto histórico del clima sobre la dinámica epidémica del dengue. El número de rezagos es un parámetro configurable.
...
*   **Ruta:** `data/dengue_data_v2_ocur.csv`
*   **Formato:** Delimitado por tuberías (`|`), codificación UTF-8.
*   **Contenido:** 202,176 registros y 34 columnas. Columnas de ubicación, fecha, casos confirmados y variables climáticas/ambientales con sus respectivos rezagos temporales (lags).

## Sistema de Logs
Para trazabilidad y depuración, el script reporta su actividad en:

*   **Archivo:** `project_logs.log` (ubicado en la raíz del proyecto).
*   **Detalle:** Cada ejecución registra timestamps, el inicio y fin de cada etapa, errores críticos y metadatos del dataset generado.
